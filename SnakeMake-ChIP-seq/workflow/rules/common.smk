...
container: "mambaorg/micromamba:1.5.1"
min_version("7.25.0")
report: "../report/workflow.rst"

# =====================================================
# validate config.yaml file and samples.tsv file
# =====================================================
...
def get_se_pe_branches_input(wildcards):
    if config["single_end"]:
        return "results/bamtools_filtered/{sample}.sorted.bam".format(
    ...
    if wildcards.read == "0" or wildcards.read == "1":
        if is_sra_se(wildcards.sample, wildcards.unit):
            return expand(
                ...
                accession=units.loc[
                    (wildcards.sample, wildcards.unit), "sra_accession"
                ],
            )
...
def get_fastqs(wildcards):
    ...
    if is_sra_se(wildcards.sample, wildcards.unit):
        ...
        return expand(
            [
                "resources/ref/sra-pe-reads/{accession}.1.fastq",
                "ref/sra-pe-reads/{accession}.2.fastq",
            ],
            accession=units.loc[(wildcards.sample, wildcards.unit), "sra_accession"],
        )
...
def get_plot_homer_annotatepeaks_input():
    return expand(
        "results/homer/annotate_peaks/{sam_contr_peak}_peaks.annotatePeaks.txt",
        sam_contr_peak=get_sample_control_peak_combinations_list(),
    )
...
def get_samtools_view_filter_input(wildcards):
    return [
        ...
        "resources/ref/blacklist.sorted.complement".format(
            prefix="chr{chr}_".format(chr=chromosome) if chromosome else "", build=build
        ),
    ]
...
def get_multiqc_input(wildcards):
    ...
    for sample, unit in units.index:
        ...
        if is_single_end(sample, unit):
            ...
            multiqc_input.extend(
                expand(
                    ["logs/cutadapt/{sample}-{unit}.se.log"], sample=sample, unit=unit
                )
            )
        else:
            multiqc_input.extend(
                expand(
                    ["logs/cutadapt/{sample}-{unit}.pe.log"], sample=sample, unit=unit
                )
            )
    ...
    for sample in samples.index:
        ...
        if config["params"]["deeptools-plots"]["activate"]:
            multiqc_input.extend(
                expand(["results/deeptools/plot_profile_data.tab"], sample=sample)
            )
        ...
        if config["params"]["lc_extrap"]["activate"]:
            multiqc_input.extend(
                expand(["results/preseq/{sample}.lc_extrap"], sample=sample)
            )
                ...
                if f.read().strip() != "":
                    ...
                    if do_consensus_peak:
                        for antibody in samples["antibody"]:
                            if exists_multiple_groups(antibody) or exists_replicates(
                                antibody
                            ):
                                wanted_input.extend(
                                    expand(
